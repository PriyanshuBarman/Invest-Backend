...
export const subtractUserPortfolio = async ({
  userId,
  redemptionAmt,
  portfolioType,
  costBasis = null,
  ...
  if (redemptionAmt === userPortfolio.totalMv.toNumber()) {
    return await userPortfolioRepo.delete({ id: userPortfolio.id });
  }

  await userPortfolioRepo.update(
    { id: userPortfolio.id },
    calculatePortfolioAfterSell(userPortfolio, redemptionAmt, costBasis)
  );
};
